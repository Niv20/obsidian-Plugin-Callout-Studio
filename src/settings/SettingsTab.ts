...
export class CalloutStudioSettingsTab extends PluginSettingTab {
	...
	private renderCalloutTypesSection(containerEl: HTMLElement): void {
		...
		// Action buttons row
		const actionsEl = containerEl.createDiv({ cls: "callout-studio-actions" });
		...
		const importBtn = actionsEl.createEl("button", { text: "Import from CSS snippet" });
	...
	private renderCalloutRow(containerEl: HTMLElement, def: CalloutDefinition, isBuiltIn: boolean): void {
		const row = containerEl.createDiv({ cls: "callout-studio-row" });

		// Icon preview
		const iconEl = row.createDiv({ cls: "callout-studio-row-icon" });
		try {
			setIcon(iconEl, def.icon.type === "lucide" ? def.icon.value : "pencil");
		} catch {
			iconEl.textContent = "📝";
		}
		...
		infoEl.createDiv({ cls: "callout-studio-row-name", text: def.displayName });
		...
		const lightSwatch = colorsEl.createDiv({
			...
			attr: { "aria-label": `Light: ${def.colorLight}`, title: `Light: ${def.colorLight}` },
		});
		lightSwatch.style.backgroundColor = def.colorLight;
		lightSwatch.createSpan({ cls: "callout-studio-swatch-label", text: "L" });

		const darkSwatch = colorsEl.createDiv({
			cls: "callout-studio-color-swatch",
			attr: { "aria-label": `Dark: ${def.colorDark}`, title: `Dark: ${def.colorDark}` },
		});
		darkSwatch.style.backgroundColor = def.colorDark;
		darkSwatch.createSpan({ cls: "callout-studio-swatch-label", text: "D" });
		...
		editBtn.addEventListener("click", async () => {
			if (isBuiltIn) {
				...
				const editorModal = new CalloutEditor(this.plugin, { ...def, builtIn: false });
	...
	private renderPopupSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.setDesc("Show a semi-transparent floating panel anchored to the right-click context menu. It shows all available callout types and lets you quickly insert, convert, or edit callouts without opening the settings panel.")
			.addToggle((t) => t.setValue(popup.enabled).onChange(async (v) => {
				popup.enabled = v;
				await this.plugin.saveSettings();
			}));

		new Setting(containerEl)
			.setName("Popup position")
			.addDropdown((d) => d
				.addOptions({ "top-left": "Top-left", "top-right": "Top-right", cursor: "Cursor" })
		...
		new Setting(containerEl)
			.setName("Show icons in popup")
			.addToggle((t) => t.setValue(popup.showIcons).onChange(async (v) => {
				popup.showIcons = v;
				await this.plugin.saveSettings();
			}));
		...
		new Setting(containerEl)
			...
			.addToggle((t) => t.setValue(popup.showColorDots).onChange(async (v) => {
		...
		new Setting(containerEl)
			...
			.addText((t) => t
				.setValue(String(popup.maxItems))
				.onChange(async (v) => {
		...
		new Setting(containerEl)
			...
			.addToggle((t) => t.setValue(popup.showConvertSubmenu).onChange(async (v) => {
		...
		new Setting(containerEl)
			...
			.addToggle((t) => t.setValue(popup.showEditButton).onChange(async (v) => {
				popup.showEditButton = v;
				await this.plugin.saveSettings();
			}));

		new Setting(containerEl)
			.setName("Popup animation")
			.addDropdown((d) => d
				.addOptions({ fade: "Fade", slide: "Slide", scale: "Scale", none: "None" })
	...
	private renderAutocompleteSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.addToggle((t) => t.setValue(autocomplete.enabled).onChange(async (v) => {
		...
		new Setting(containerEl)
			...
			.addToggle((t) => t.setValue(autocomplete.showIconPreviews).onChange(async (v) => {
				autocomplete.showIconPreviews = v;
				await this.plugin.saveSettings();
			}));

		new Setting(containerEl)
			.setName("Show color previews")
			.addToggle((t) => t.setValue(autocomplete.showColorPreviews).onChange(async (v) => {
		...
		new Setting(containerEl)
			...
			.setDesc("The autocomplete is triggered when you type this sequence")
			.addText((t) => {
	...
	private renderIconSourceSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.addToggle((t) => t.setValue(iconSources.lucide).onChange(async (v) => {
		...
		new Setting(containerEl)
			...
			.addToggle((t) => t.setValue(iconSources.material).onChange(async (v) => {
		...
		new Setting(containerEl)
			...
			.addToggle((t) => t.setValue(iconSources.customSvg).onChange(async (v) => {
		...
		new Setting(containerEl)
			...
			.addDropdown((d) => d
				.addOptions({ outlined: "Outlined", filled: "Filled", rounded: "Rounded", sharp: "Sharp" })
				.setValue(iconSources.materialStyleDefault)
				.onChange(async (v: string) => {
					iconSources.materialStyleDefault = v as typeof iconSources.materialStyleDefault;
					await this.plugin.saveSettings();
				}));

		new Setting(containerEl)
			.setName("Cache Material Icons offline")
			.setDesc("Downloads icon metadata for offline use (~500KB)")
			.addToggle((t) => t.setValue(iconSources.cacheMaterialOffline).onChange(async (v) => {
	...
	private renderColorModeSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.addDropdown((d) => d
				.addOptions({ auto: "Auto (recommended)", light: "Force light colors only", dark: "Force dark colors only" })
		...
		new Setting(containerEl)
			...
			.setDesc("Warns if icon color has low contrast against callout background")
			.addToggle((t) => t.setValue(colorMode.showContrastWarning).onChange(async (v) => {
		...
		try {
			...
			for (const file of files.files) {
				if (file.endsWith(".css")) {
					...
					const imported = this.plugin.registry.importFromCSS(content);
					totalImported += imported.length;
				}
			}

			if (totalImported > 0) {
				new Notice(`Imported ${totalImported} callout type(s) from CSS snippets.`);
			} else {
