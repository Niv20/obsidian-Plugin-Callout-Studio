 ...
 * `CSSInjector`, so the snippet cannot drift from what the plugin draws.
...
/** First bytes of a file this module wrote. */
...
/**
 * The header, carrying a fingerprint of everything below it — the line that
 * lets a later export tell "the settings changed" apart from "the user edited
 * this file", which are otherwise the same diff. Deliberately no timestamp and
 * no version: identical settings must produce identical bytes, or every export
 * would rewrite the file and sync it.
 */
function header(fingerprint: string): string {
	return (
		"/* Generated by Callout Studio — do not edit.\n" +
		" * Exporting again from Settings → Callout Studio → Import / export\n" +
		" * replaces this file.\n" +
		" *\n" +
		" * This snippet is NOT switched on automatically. Enable it under\n" +
		" * Settings → Appearance → CSS snippets, or copy it to another vault.\n" +
 ...
 */
async function classify(text: string): Promise<Existing> {
...
async function readExisting(app: App, path: string): Promise<Existing> {
	...
	try {
		...
		return await classify(await adapter.read(path));
	} catch (e) {
...
async function runExport(
	...
	try {
		...
		text = header(await sha256(body)) + body;
	} catch (e) {
