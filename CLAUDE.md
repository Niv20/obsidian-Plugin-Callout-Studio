...
- **`CSSInjector` is the only thing that branches**: a picture with `recolor: false` is emitted as `background-image` (`generateImageOverride`), not `mask-image`, because a mask is a stencil and would flatten it to a silhouette. `paintIcon` correspondingly passes `fill: "currentColor"` for those.
...
- **The picker uses `ImagePanel`, not `PackPanel`** — add/rename/delete/recolor are affordances the `IconPack` contract deliberately has no room for.
