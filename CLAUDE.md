...
- **`CSSInjector` is the only thing that branches**: a picture not following the callout is emitted as `background-image` (`generateImageOverride`), not `mask-image`, because a mask is a stencil and would flatten it to a silhouette. `paintIcon` correspondingly passes `fill: "currentColor"` for those. `cacheVariant` keys on `icon.recolor` for the same reason — two callouts sharing a picture must not share a render key.
